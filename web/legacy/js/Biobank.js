$().ready(function () {
    ...
    $("div#order_div button.order_samples").click(function () {
        ...
        if ($('div#Layout_navigation ul li a[href=#agreement_biobank]:visible').size() == 1) {
            alert('Already an order in process');
            return false;
        }

        // IN CASE ORDER ALREADY IN PROCESS
        if ($($('div#Layout_navigation ul li a[href=#order_samples]:visible')).size() == 1) {
            alert('Already an order in process');
            return false;
        }


        if ($('div#search_selected_samples table').size() == 0) {
