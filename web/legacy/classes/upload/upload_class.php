...
class Upload 
{
	...
	public function __construct($folderdir,$overwrite = false)
	{
		...
		if($this->checkDir($folderdir)==true){$this->folderdir = $folderdir;}
	...
	{
		if(is_integer($size)){ $this->maxsize=$size;}
	}
	public function setMaxFilenameSize($size)
	{
		if(is_integer($size)){ $this->maxfilename = $size;}
	}
	...
	{$this->isError = true;$this->errormessage = $this->errormessages[14];return false;}
	...
	if (move_uploaded_file($filetmp, $filenew)==true) 
	{
					...
					{ 
					$this->isError =true;$this->errormessage = $this->errormessages[15];return false; 
					}
					
	}
	else 
	{ $this->isError = true;$this->errormessage = $this->errormessages[16];return false;}
	...
	{
	// check html errors
	if($file['error'] != 0) {$this->isError = true;$this->errormessage = $this->errormessages[$file['error']]; return false;}
	
	// compare file size to limit
	
	// might be removed later since a check is already performed by php on form submission
	if($this->maxsize < $this->getSize($file) ){$this->isError = true;$this->errormessage = $this->errormessages[10];return false;}
	
	if($this->CheckExt($file)== false){$this->isError = true;$this->errormessage=$this->errormessages[11];return false;}
	
	$filename = $this->setFilename($file);
	// check filename size
	if(strlen($filename)>$this->maxfilename){$this->isError = true;$this->errormessage = $this->errormessages[12];return false;}
	// check if file already exists
	if($this->overwrite == false && $this->doesFileExists($filename) == true )
	{$this->isError = true;$this->errormessage = $this->errormessages[13];return false; }
	return $filename;
	}
	...
	protected function CheckExt($file)
	{
		...
		{
			if(in_array($extfile,$extension)){ return true;}
		}
		return false;
	}
	 ...
	 * @return bool
	 */
	protected function doesFileExists($filename){  return file_exists($this->folderdir.$filename);}
	/**
	 * 
	 *
	 * @param array $file
...
class Uploads extends Upload 
{
	...
	public function __construct($folderdir,$action = false,$overwrite = false)
	{
		if($action == false){ $this->action = $_SERVER['PHP_SELF'];}
		else { $this->action = $action;}
		
		Upload::__construct($folderdir,$overwrite);
		// init files to upload
		if($this->InitFiles()==false){return false;};
	}
	
	public function setClass($in) { if(is_string($in) && strlen($in)>0){ $this->formclass = $in;} }
	...
	{
		if(count($this->UploadedFiles)==0){ return false;}
		$tmp = array();
		...
		if(count($tmp)>0){ return $tmp; } else { return false;}
	...
	{	
		...
		{ $this->isError = true;$this->errormessage = $this->errormessages[9];return false;
		}
	}
	...
	public function UploadFiles()
	{
		...
		if(isset($_POST['active_tab']) && $_POST['active_tab'] != $this->formclass){ return false;}
		
		if($this->isError==true){return false;} // in case no files to upload -- just exit the function
	...
	public function __toString()
	{
			...
			$tostring .= "<div class='initinput'><div><input type='file' name='upload[]' size='30'/>".
						 "<button type='button' class='del'><img src='/img/cross.png' alt='Del'/></button></div></div>";	
			...
			$tostring .="<label for='inputs'>Select a file</label>";
			$tostring .="<div id='inputs'>";
			if(count($this->UploadedFiles) == 0)
			{
			$tostring .= "<div><input type='file' name='upload[]' size='30'/><button type='button' class='del'><img src='/img/cross.png' alt='Del' /></button></div>";
			}
			else 
			{
			...
			$tostring .="<button type='button' class='add'><img src='/img/add.png' alt='Add' /></button>";
			$tostring .="<button class='submit' type='submit' name='submit' value='submit'><img src='/img/accept.png' alt='Accept'/></button>";
			$tostring .= "</div>";
