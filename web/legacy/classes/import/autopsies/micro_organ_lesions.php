...
$css = "css/autopsy_import/autopsy_macro_organ_lesions.css";

$js = "js/autopsy_import/autopsy_macro_organ_lesions.js";
...
// get all microscopic lesions ( sane organs are excluded=>therefore inner join on the lesion values)


$sql = "select a.seqno as oln_seqno, b.seqno as lte_seqno, b.ogn_code organ,b.processus Process,a.description, CASE WHEN EXISTS (SELECT 1 FROM lesions2sample l2s WHERE l2s.oln_seqno=a.seqno       )     THEN 1     ELSE 0   END AS hasSample  from organ_lesions a, lesion_types b where b.seqno = a.lte_seqno and a.ncy_ese_seqno = $necropsy_seqno and b.processus != 'NA' and scale = 0";
...
$sql = "SELECT b.ncy_ese_seqno,c.ogn_code organ,   c.processus Process,   d.name,   a.value,   b.seqno FROM lesion_values a,   organ_lesions b,   lesion_types c,   parameter_methods d WHERE a.oln_seqno = b.seqno AND c.seqno       = b.lte_seqno AND d.seqno       = a.pmd_seqno and b.ncy_ese_seqno = $necropsy_seqno and scale         = 0";
    ...
    foreach ($lesion_to_insert as $lesion) {
        ...
        if ($val->getStatus()) // the status remain true, i.e either the lesion_type existed already or it has been entered successfully
        {
            if (!isset($lesion['UPD'])) { //if this is actually an update because the lesion type changed, don't do anythings
                $sql = "insert into organ_lesions (lte_seqno,ncy_ese_seqno,description,scale) values ($lte_seqno,$necropsy_seqno,:description,0)";
