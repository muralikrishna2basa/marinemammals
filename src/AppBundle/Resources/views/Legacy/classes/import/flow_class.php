...
class Flow
{
	...
	public function __construct($server = false,$flowname = false,$db = false,$auth = false)
	{
		...
		if($flowname!=false&&is_string($flowname)&& strlen($flowname)>0){ $this->flowname .= '_'.$flowname;}
		...
		if($server == false) { $this->server = $_SERVER['PHP_SELF'];}
	...
	{
		if(is_array($bar)){ $this->statebar = $bar;}
	}
	/**
...
//	}
	...
	public function addJsCss($data)
	{
		...
		{
			if(!array_key_exists($name,$this->includesname) || !is_array($jscss)){ continue;}
			if(array_key_exists('css',$jscss)){ $this->includesname[$name]['css'] = $jscss['css'];}
			if(array_key_exists('js',$jscss)){ $this->includesname[$name]['js'] = $jscss['js'];}
		}
	...
	public function getCurrentJsCss(){ return $this->getJsCss($this->flow);}

	public function getPrevJsCss(){ return $this->getJsCss(($this->flow-1)%count($this->includes));}
	
	public function getNextJsCss(){ return $this->getJsCss(($this->flow+1)%count($this->includes));}
	...
	{
		$this->flow +=1;
		if($this->flow > count($this->includes)-1) { $this->flow = count($this->includes)-1;}
		$this->save();
		return $this->flow; 
	}
	public function init()
	...
	public function getButtonImage($name)
	{
		switch($name)
		{
			case 'refresh':return "<img alt='refresh' src='/legacy/img/Container_Localizations/refresh.png'></img>";
			case 'home':return "<img alt='home' src='/legacy/img/Container_Localizations/stop.png'></img>";
			case 'next':return "<img alt='NEXT' src='/legacy/img/next.png'></img>";
			case 'previous':return "<img alt='PREV' src='/legacy/img/previous.png'></img>";
		}
	}
		...
		|| (isset($_POST['finish']) && $_POST['finish']==''))
	{
		...
		if($cntincludes == 0) { return false;} // in case no includes has previously been setup
	//	if($cntincludes == 1) { $included = 0;}
		else { $included = $this->current()%($cntincludes);}
	...
	{
		if(isset($_POST) && isset($_POST['button'])){ unset($_POST['button']);}
	}
	/**
