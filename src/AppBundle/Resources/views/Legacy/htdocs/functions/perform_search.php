...
if(isset($_GET['search_json'])){ $search_json = $_GET['search_json'];} else { $search_json = false;}
...
if(isset($_GET['check']) && is_string($_GET['check']) && strlen($_GET['check'])>0){ $check = explode(',',stripslashes($_GET['check']));}else { $check = array();}

if(isset($_GET['uncheck']) && is_string($_GET['uncheck']) && strlen($_GET['uncheck'])>0){ $uncheck = explode(',',stripslashes($_GET['uncheck']));} else { $uncheck = array();}
...
if(isset($_SESSION['samples'])==true && is_array($_SESSION['samples'])==true)
{
	...
	{
		if(!in_array($item,$samples_tmp)){ $samples_tmp[] = $item;}
	}
...
$samples->renderer->check = $_SESSION['samples'];
...
{	
	$check = $samples->check;
	if(is_array($check) && in_array($samples->fullpk,$check) == true ) {return 'key';}
	else { return 'value';} 
}
...
elseif(isset($_SESSION['samples']) && count($_SESSION['samples'])>0)
{
